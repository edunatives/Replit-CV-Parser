...
function getPath(obj: unknown, ...paths: string[]): unknown {
  for (const path of paths) {
    ...
    for (const key of path.split(".")) {
      ...
      // Try exact, camelCase, snake_case
...
function transformToLite(raw: unknown, audience: AudienceType): LiteOutput {
  ...
  const output: LiteOutput = {
    ...
    topIssues: safeArr(getPath(data, "topIssues", "top_issues")).slice(0, 3).map((i: any) => ({
      code: safeStr(i.code, "A1"),
      issue: safeStr(i.issue || i.detail, "Unknown issue"),
      severity: (i.severity || "medium") as any,
      count: safeNum(i.count, 1),
      fix: i.fix,
    })),
    topStrengths: safeArr(getPath(data, "topStrengths", "top_strengths")).slice(0, 3).map((s: any) => ({
      code: safeStr(s.code, "D1"),
      strength: safeStr(s.strength || s.detail, "Unknown strength"),
    })),
  ...
  if (audience === "HR") {
    ...
    if (hire) output.hireRecommendation = hire as any;
  }
...
function transformToStandard(raw: unknown, audience: AudienceType): StandardOutput {
  ...
  const output: StandardOutput = {
    ...
    cvSummary: {
      ...
      seniorityLevel: (cvSum.seniorityLevel || cvSum.seniority_level || "Mid") as any,
    ...
    jdSummary: jdSum ? {
      ...
      seniorityLevel: (jdSum.seniorityLevel || jdSum.seniority_level || "Mid") as any,
    ...
    skillMatch: skillMatch ? {
      tier1: { matched: safeNum((skillMatch.tier1 as any)?.matched), total: safeNum((skillMatch.tier1 as any)?.total), score: safeNum((skillMatch.tier1 as any)?.score) },
      tier2: { matched: safeNum((skillMatch.tier2 as any)?.matched), total: safeNum((skillMatch.tier2 as any)?.total), score: safeNum((skillMatch.tier2 as any)?.score) },
      tier3: { matched: safeNum((skillMatch.tier3 as any)?.matched), total: safeNum((skillMatch.tier3 as any)?.total), score: safeNum((skillMatch.tier3 as any)?.score) },
    ...
    bulletHealth: {
      ...
      distribution: {
        excellent: safeNum((bullet.distribution as any)?.excellent),
        good: safeNum((bullet.distribution as any)?.good),
        fair: safeNum((bullet.distribution as any)?.fair),
        poor: safeNum((bullet.distribution as any)?.poor),
      },
    ...
    improvements: {
      critical: safeArr(impr.critical).map((i: any) => typeof i === "string" ? { code: "A1", priority: "critical" as const, action: i, impact: "High", effort: "Medium" } : i),
      high: safeArr(impr.high).map((i: any) => typeof i === "string" ? { code: "A1", priority: "high" as const, action: i, impact: "Medium", effort: "Medium" } : i),
      medium: safeArr(impr.medium).map((i: any) => typeof i === "string" ? { code: "A1", priority: "medium" as const, action: i, impact: "Low", effort: "Low" } : i),
      scorePotential: {
        current: lite.scores.overall,
        afterCritical: safeNum((impr.scorePotential as any)?.afterCritical, lite.scores.overall + 5),
        afterAll: safeNum((impr.scorePotential as any)?.afterAll || (impr.scorePotential as any)?.afterFixes, lite.scores.overall + 10),
        ceiling: safeNum((impr.scorePotential as any)?.ceiling, lite.scores.overall + 15),
      },
    },
  ...
  if (audience === "STUDENT") {
    output.alternativeRoles = safeArr(getPath(data, "alternativeRoles", "alternative_roles")).map((r: any) => ({
      role: safeStr(r.role),
      fitScore: safeNum(r.fitScore || r.fit_score),
      reason: safeStr(r.reason, "Good fit based on skills"),
    }));
    const nextSteps = getPath(data, "nextSteps", "next_steps");
    output.nextSteps = typeof nextSteps === "object" && nextSteps ? nextSteps as any : {
    ...
    output.verificationItems = safeArr(getPath(data, "verificationItems", "topVerificationItems")).map((v: any) => 
      typeof v === "string" ? { item: v, priority: "medium" as const, reason: "Requires verification" } : v
    );
    output.interviewQuestions = safeArr(getPath(data, "interviewQuestions")).map((q: any) => ({
      question: safeStr(q.question),
      probing: safeStr(q.probing, "Follow up on details"),
      redFlag: safeStr(q.redFlag, "Vague or inconsistent answers"),
    }));
  }
...
function transformToFull(raw: unknown, audience: AudienceType): FullOutput {
  ...
  return {
    ...
    audience,
    generatedAt: new Date().toISOString(),
    cvAnalysis: (getPath(data, "cvAnalysis", "cv_analysis") || {}) as any,
    jdAnalysis: getPath(data, "jdAnalysis", "jd_analysis") as any,
    studentAnalysis: audience === "STUDENT" ? getPath(data, "studentAnalysis", "student_analysis") as any : undefined,
    hrAnalysis: audience === "HR" ? getPath(data, "hrAnalysis", "hr_analysis") as any : undefined,
  };
}
...
export class CvIntelligenceChain {
  ...
  async analyze(request: {
    ...
    try {
      ...
      // Return fallback lite output on error
