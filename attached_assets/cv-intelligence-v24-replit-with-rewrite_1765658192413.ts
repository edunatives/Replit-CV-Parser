 ...
 * With FULL REWRITE SUPPORT
 * 
 * Features:
 * - Per-bullet rewrites with projected scores
 * - Summary rewrite
 * - Skills section rewrite
 * - Full CV rewrite option
 */

// ============================================================================
// SYSTEM PROMPT v2.4 WITH REWRITE SUPPORT
// ============================================================================
...
// ============================================================================
// USER PROMPT WITH REWRITE SUPPORT
// ============================================================================
  ...
  "cvAnalysis": {
    ...
    "sections": {
      "contact": {
        ...
        "suggestions": [{"priority": 1, "action": "<action>", "suggestedText": "<text>", "effort": "<effort>", "impact": "<+X pts>"}],
        "rewrite": {
          "needed": <boolean>,
            ...
            "bullets": [
              {
                ...
                "rewrite": {
                  ...
                  "changes": [
                    ...
                    "Added result: reducing costs by [Y]%",
                    "Added scope: across [Z] teams"
                  ],
                  "projectedScore": <0-100>,
                  "placeholders": ["[X]%", "[Y]%", "[Z]"]
                }
      ...
      "certifications": {
        ...
        "rewrite": {
          "needed": <boolean>,
          "suggestions": ["<update expired cert status>", "<add certification date>"]
        }
      }
    },
      ...
      "roles": [
        {
          ...
          "rewrittenBullets": [
            "<Strong verb> + <what you did> + <quantified scope> + <quantified result>",
            "<Strong verb> + <achievement> + <impact with numbers>"
          ]
        }
      ]
    },
...
// ============================================================================
// REWRITE-ONLY PROMPT (for faster rewrite generation)
// ============================================================================

export function buildRewriteOnlyPrompt(cv: string, jd?: string): string {
...
// ============================================================================
// BULLET REWRITE PROMPT (for single bullet improvements)
// ============================================================================

export function buildBulletRewritePrompt(bullet: string, context?: { role?: string; company?: string; jd?: string }): string {
...
export function buildSummaryRewritePrompt(summary: string, context?: { name?: string; title?: string; years?: number; jd?: string }): string {
...
## OUTPUT
{
  ...
  "rewrites": [
    {
      "style": "conservative",
      "text": "<minimal changes rewrite>",
      "score": <n>
    },
    {
      "style": "balanced",
      "text": "<moderate rewrite following formula>",
      "score": <n>
    },
    {
      "style": "bold",
...
// ============================================================================
// HELPER: CREATE REPLIT PROMPT
// ============================================================================

export function createReplitPrompt(
  cv: string, 
  jd?: string, 
  audience: 'STUDENT' | 'HR' = 'STUDENT',
  includeFullRewrite: boolean = true
  ...
  return {
    ...
    userPrompt: buildUserPrompt(cv, jd, audience, includeFullRewrite)
  };
}

// ============================================================================
// HELPER: REWRITE ONLY (faster, focused on rewrites)
// ============================================================================

export function createRewritePrompt(cv: string, jd?: string): {
  systemPrompt: string;
  userPrompt: string;
} {
  return {
    systemPrompt: `You are CV Rewrite Specialist. Transform weak CVs into strong ones.
Rules:
- Use strong action verbs (Led, Delivered, Architected)
- Add quantification with [X] placeholders
- Every bullet needs a result/impact
- Remove ghost skills (no evidence)
- Remove generic phrases
- Output valid JSON only`,
    userPrompt: buildRewriteOnlyPrompt(cv, jd)
  };
}
