...
/**
 * CV Type - 6 categories
 */
export type CVType = 
  | 'student'       // Currently enrolled, no/only internship experience
  | 'fresh_grad'    // Graduated within 2 years, 0-2 years experience
  | 'researcher'    // PhD/postdoc/academic with publications/research focus
  | 'professional'  // 3-7 years industry experience, mid-level
  | 'expert'        // 8+ years, senior IC/specialist, deep technical track
  | 'executive';    // C-level, VP, Director, management/leadership track
...
export interface DetectionSignals {
  ...
  // Expert signals (Senior IC / Specialist)
  ...
  // Executive signals (Management / Leadership)
...
const PL_KEYWORDS = [
  'p&l', 'profit and loss', 'profit & loss', 'revenue', 'budget',
  'cost center', 'profit center', 'financial responsibility',
  'top line', 'bottom line', 'ebitda', 'margin', 'roi'
];
...
/**
 * Extract budget/revenue numbers
 */
...
/**
 * Check if has only internship experience
 */
...
/**
 * Check for expert-level signals
 */
...
export function detectCVType(cvText: string): CVTypeResult {
  ...
  // ----- PROFESSIONAL SCORING (3-7 years, mid-level) -----
  ...
  if (signals.totalYearsExperience >= 10) scores.professional -= 30; // Too senior
  if (signals.hasCLevelTitle || signals.hasVPTitle) scores.professional -= 40; // Executive
  if (signals.hasPrincipalStaffTitle) scores.professional -= 30; // Expert
  
  // ----- EXPERT SCORING (8+ years, senior IC/specialist) -----
  ...
  // Penalties for expert
  if (signals.totalYearsExperience < 6) scores.expert -= 40;
  if (signals.hasCLevelTitle || signals.hasVPTitle) scores.expert -= 30; // More executive
  if (signals.hasManagementTitle && signals.teamSizeMax && signals.teamSizeMax > 20) scores.expert -= 25; // Manager track
  if (signals.isCurrentlyEnrolled) scores.expert -= 50;
  
  // ----- EXECUTIVE SCORING (C-level, VP, Director, management) -----
